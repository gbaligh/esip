...
#ifdef PACKAGE
#define ES_LOG_DEFAULT_MODULE     PACKAGE
#else
...
static void es_log_vprintf(es_log_t * handler, const unsigned int level, const char const * fmt, va_list ap)
{
  if (handler == NULL) {
    ESIP_TRACE(ESIP_LOG_ERROR, "bad arguments");
    return;
  ...
  fprintf(stderr, "[%s]-", handler->module);
...
void es_log_trace(es_log_t * handler, unsigned int level, const char const * format, ...)
{
  ...
  va_start(ap, format);
  es_log_vprintf(handler, level, format, ap);
  va_end(ap);
}
...
void es_log_emerg(es_log_t * handler, const char const * format, ...)
{
  ...
  va_start(ap, format);
  es_log_vprintf(handler, ESIP_LOG_EMERG, format, ap);
  va_end(ap);
}
...
void es_log_alert(es_log_t * handler, const char const * format, ...)
{
  ...
  va_start(ap, format);
  es_log_vprintf(handler, ESIP_LOG_ALERT, format, ap);
  va_end(ap);
}
...
void es_log_crit(es_log_t * handler, const char const * format, ...)
{
  ...
  va_start(ap, format);
  es_log_vprintf(handler, ESIP_LOG_CRIT, format, ap);
  va_end(ap);
}
...
void es_log_error(es_log_t * handler, const char const * format, ...)
{
  ...
  va_start(ap, format);
  es_log_vprintf(handler, ESIP_LOG_ERROR, format, ap);
  va_end(ap);
}
...
void es_log_warning(es_log_t * handler, const char const * format, ...)
{
  ...
  va_start(ap, format);
  es_log_vprintf(handler, ESIP_LOG_WARNING, format, ap);
  va_end(ap);
}
...
void es_log_notice(es_log_t * handler, const char const * format, ...)
{
  ...
  va_start(ap, format);
  es_log_vprintf(handler, ESIP_LOG_NOTICE, format, ap);
  va_end(ap);
}
...
void es_log_info(es_log_t * handler, const char const * format, ...)
{
  ...
  va_start(ap, format);
  es_log_vprintf(handler, ESIP_LOG_INFO, format, ap);
  va_end(ap);
}
...
void es_log_debug(es_log_t * handler, const char const * format, ...)
{
  ...
  va_start(ap, format);
  es_log_vprintf(handler, ESIP_LOG_DEBUG, format, ap);
  va_end(ap);
}
